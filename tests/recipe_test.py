...
check(
    "alias בולונז",
    a.post(f"{BASE}/shopping/recipe", json={"dish": "בא לי בולונז"}).json()["dish"]
    == "פסטה בולונז",
)
check(
    "שקשוקה הערב",
    a.post(f"{BASE}/shopping/recipe", json={"dish": "רוצים שקשוקה הערב"}).json()["dish"]
    == "שקשוקה",
)
check(
    "מגדרה בלי גרש",
    a.post(f"{BASE}/shopping/recipe", json={"dish": "מגדרה"}).json()["dish"] == "מג'דרה",
)
...
check(
    "unknown dish handled",
    r.status_code == 404 or (r.status_code == 200 and r.json()["source"] == "ai"),
    f"{r.status_code} {r.text[:120]}",
)

# --- ולידציות ---
check(
    "empty dish rejected", a.post(f"{BASE}/shopping/recipe", json={"dish": "  "}).status_code == 400
)
check(
    "filler-only rejected",
    a.post(f"{BASE}/shopping/recipe", json={"dish": "בא לי לאכול משהו טעים"}).status_code
    in (400, 404),
)

# --- הוספה בכמות לרשימה ---
items = [
    {"name": i["name"], "note": (i["note"] + " · " if i["note"] else "") + "לפסטה בולונז"}
    for i in d["ingredients"][:5]
]
...
# --- כפילויות מדולגות ---
r = a.post(
    f"{BASE}/shopping/bulk", json={"items": items + [{"name": "פריט חדש לגמרי", "note": ""}]}
)
...
check(
    "bulk too many rejected",
    a.post(f"{BASE}/shopping/bulk", json={"items": [{"name": "x"}] * 41}).status_code == 400,
)
