...
check("post pinned note", b.post(f"{BASE}/bulletin", json={"content": "קוד ה-Wi-Fi החדש: 12345678", "is_pinned": True}).ok)
...
check("then newest first", notes[1]["content"] == "נגמר סבון כלים" and notes[2]["content"] == "הטכנאי מגיע מחר")
check("author name attached", notes[0]["author"] == "בדיקה-ב" and notes[1]["author"] == "בדיקה-א")

# --- מחיקה: לוח משותף — כל שותף מוחק כל מודעה ---
wifi_id = notes[0]["id"]
tech_id = notes[2]["id"]
check("member deletes someone else's note", b.delete(f"{BASE}/bulletin/{tech_id}").ok)   # ב' מוחק של א'
...
# --- ולידציות ---
check("empty content rejected", a.post(f"{BASE}/bulletin", json={"content": "   "}).status_code == 400)
check("too-long content rejected", a.post(f"{BASE}/bulletin", json={"content": "א" * 301}).status_code == 400)

# --- בידוד בין דירות ---
c = requests.Session()
c.post(f"{BASE}/register", json={"name": "זר", "email": "testc@example.com", "password": "secret3"}).raise_for_status()
...
check("state includes bulletin", any(n["content"] == "נגמר סבון כלים" for n in st.get("bulletin", [])))
