...
def has(items, id_prefix, severity=None):
    return any(n["id"].startswith(id_prefix) and (severity is None or n["severity"] == severity)
               for n in items)
...
a.post(f"{BASE}/expenses", json={"descr": "קניות", "amount": 100, "category_id": cats[0]["id"],
                                 "date": TODAY.isoformat(), "payer_id": st["members"][0]["id"],
                                 "split_type": "equal"}).raise_for_status()
a.patch(f"{BASE}/categories/{cats[1]['id']}", json={"budget": 100}).raise_for_status()
a.post(f"{BASE}/expenses", json={"descr": "כמעט בתקרה", "amount": 90, "category_id": cats[1]["id"],
                                 "date": TODAY.isoformat(), "payer_id": st["members"][0]["id"],
                                 "split_type": "personal"}).raise_for_status()
na = notifs(a)
...
a.post(f"{BASE}/bills", json={"name": "חשבון בדיקה", "amount": 123, "due_day": TODAY.day,
                              "category_id": cats[0]["id"]}).raise_for_status()
na = notifs(a)
...
a.post(f"{BASE}/personal", json={"descr": "פרטי", "amount": 50, "date": TODAY.isoformat()}).raise_for_status()
...
na, nb = notifs(a), notifs(b)
check("urgent shopping for all members", has(na, "shopping-urgent-") and has(nb, "shopping-urgent-"))
...
check("every notification has a target tab",
      all(n["tab"] in ("dash", "expenses", "personal", "budgets", "shopping", "bills", "chores") for n in na))
