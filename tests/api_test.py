...
# --- הרשמה ואימות ---
r = a.post(f"{BASE}/register", json={"name": "בדיקה-א", "email": "testa@example.com", "password": "secret1"})
check("register A", r.ok, r.text)
r = requests.post(f"{BASE}/register", json={"name": "כפול", "email": "testa@example.com", "password": "secret1"})
...
r = b.post(f"{BASE}/register", json={"name": "בדיקה-ב", "email": "testb@example.com", "password": "secret2"})
check("register B", r.ok, r.text)
check("register weak password rejected",
      requests.post(f"{BASE}/register", json={"name": "x", "email": "t@t.co", "password": "123"}).status_code == 400)
...
check("join with bad code rejected", b.post(f"{BASE}/household/join", json={"code": "XXXXXX"}).status_code == 400)
...
# --- הוצאות וחלוקות ---
r = a.post(f"{BASE}/expenses", json={"descr": "סופר", "amount": 100, "category_id": cat,
                                     "date": f"{MONTH}-08", "payer_id": A, "split_type": "equal"})
check("expense equal", r.ok, r.text)
st = a.get(f"{BASE}/state?month={MONTH}").json()
check("equal: A +50", bal(st, A) == 50, str(st["balances"]))
check("equal: B -50", bal(st, B) == -50)

r = b.post(f"{BASE}/expenses", json={"descr": "אוזניות", "amount": 40, "category_id": cat,
                                     "date": f"{MONTH}-08", "payer_id": B, "split_type": "personal"})
check("expense personal", r.ok, r.text)
st = a.get(f"{BASE}/state?month={MONTH}").json()
check("personal: balances unchanged", bal(st, A) == 50 and bal(st, B) == -50)

r = b.post(f"{BASE}/expenses", json={"descr": "שוק", "amount": 90, "category_id": cat,
                                     "date": f"{MONTH}-08", "payer_id": B, "split_type": "custom",
...
r = a.post(f"{BASE}/expenses", json={"descr": "רע", "amount": 100, "category_id": cat,
                                     "date": f"{MONTH}-08", "payer_id": A, "split_type": "custom",
                                     "shares": {str(A): 10, str(B): 20}})
check("custom bad sum rejected", r.status_code == 400)

# --- הצעת העברות וסגירה ---
t = st["transfers"]
check("transfer suggested A->B 10", len(t) == 1 and t[0]["from_id"] == A and t[0]["to_id"] == B and t[0]["amount"] == 10, str(t))
r = a.post(f"{BASE}/settlements", json={"from_id": A, "to_id": B, "amount": 10, "date": f"{MONTH}-08"})
...
st = a.get(f"{BASE}/state?month={MONTH}").json()
check("unpay removed expense + balance", bal(st, A) == 0 and not any(e["descr"] == "חשמל" for e in st["expenses"]))

# --- קניות ---
a.post(f"{BASE}/shopping", json={"name": "חלב", "note": "2 קרטונים", "urgent": True})
a.post(f"{BASE}/shopping", json={"name": "לחם", "urgent": False})
st = a.get(f"{BASE}/state?month={MONTH}").json()
check("shopping added by name", any(s["name"] == "חלב" and s["added_by"] == "בדיקה-א" for s in st["shopping"]))
milk = [s for s in st["shopping"] if s["name"] == "חלב"][0]
b.patch(f"{BASE}/shopping/{milk['id']}")
r = b.post(f"{BASE}/shopping/finish", json={"expense": {"amount": 50, "payer_id": B, "category_id": cat, "date": f"{MONTH}-08"}})
...
check("shopping expense created", len(exp) == 1 and exp[0]["amount"] == 50 and "חלב" in exp[0]["descr"])
