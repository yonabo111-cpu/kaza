...
b.post(f"{BASE}/register", json={"name": "בדיקה-ב", "email": "testb@example.com", "password": "secret2"}).raise_for_status()
...
# --- א' מוסיף הוצאה פרטית ---
r = a.post(f"{BASE}/personal", json={"descr": SECRET_DESCR, "amount": 200,
                                     "category": "מתנות", "date": f"{MONTH}-08"})
...
check("sanity: secret IS in A's state", SECRET_DESCR in json.dumps(a.get(f"{BASE}/state?month={MONTH}").json(), ensure_ascii=False))
check("B personal empty", len(stb["personal"]["expenses"]) == 0 and stb["personal"]["private_total"] == 0)
check("B export has no secret", SECRET_DESCR not in json.dumps(b.get(f"{BASE}/export").json(), ensure_ascii=False))
check("A export includes own private", SECRET_DESCR in json.dumps(a.get(f"{BASE}/export").json(), ensure_ascii=False))

# --- הוצאה משותפת: החלק שלי מחושב נכון ---
a.post(f"{BASE}/expenses", json={"descr": "סופר", "amount": 100, "category_id": cat,
                                 "date": f"{MONTH}-08", "payer_id": A, "split_type": "equal"}).raise_for_status()
sta = a.get(f"{BASE}/state?month={MONTH}").json()
stb = b.get(f"{BASE}/state?month={MONTH}").json()
check("A share_total 50", sta["personal"]["share_total"] == 50, str(sta["personal"]))
check("B share_total 50", stb["personal"]["share_total"] == 50)
check("A combined chart 250", sta["personal"]["chart"][-1]["total"] == 250, str(sta["personal"]["chart"]))
...
sta = a.get(f"{BASE}/state?month={MONTH}").json()
check("private gone after delete", len(sta["personal"]["expenses"]) == 0 and sta["personal"]["private_total"] == 0)

# --- ולידציות ---
check("private bad amount rejected",
      a.post(f"{BASE}/personal", json={"descr": "x", "amount": -5, "date": f"{MONTH}-08"}).status_code == 400)
check("private bad date rejected",
      a.post(f"{BASE}/personal", json={"descr": "x", "amount": 5, "date": "לא תאריך"}).status_code == 400)
check("private requires login", requests.post(f"{BASE}/personal", json={"descr": "x", "amount": 5, "date": f"{MONTH}-08"}).status_code == 401)
