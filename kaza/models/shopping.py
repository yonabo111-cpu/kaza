...
def get_done_flag(item_id: int, household_id: int) -> Row | None:
    ...
    return (
        get_db()
        .execute("SELECT done FROM shopping WHERE id=? AND household_id=?", (item_id, household_id))
        .fetchone()
    )
...
def delete(item_id: int, household_id: int) -> None:
    ...
    get_db().execute("DELETE FROM shopping WHERE id=? AND household_id=?", (item_id, household_id))
